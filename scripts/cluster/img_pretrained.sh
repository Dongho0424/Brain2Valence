...
DONGHO="--wandb-project v2_cluster_categories --wandb-entity donghochoi --wandb-log --optimizer adamw --scheduler cosine --criterion emotic_SL1 --cat-criterion ${cat_loss}"
...
do
    ...
    do
        if [ "$subj" == "$all_subjects" ]; then
            model_name="img_pretrained_lr_${lr}_subj_1257_cluster_"
        else
            model_name="img_pretrained_lr_${lr}_subj_${subj}_cluster_"
        fi
