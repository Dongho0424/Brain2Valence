...
# DONGHO=" --wandb-project v2_cluster_categories --wandb-entity donghochoi --wandb-log --dataset-ver 2 --optimizer adamw --scheduler cosine --criterion emotic_SL1 --cat-criterion ${cat_loss}"
DONGHO=" --wandb-project v2_cluster_categories --wandb-entity donghochoi --dataset-ver 2 --optimizer adamw --scheduler cosine --criterion emotic_SL1 --cat-criterion ${cat_loss}"
    ...
    do
        ...
        do
            if [ "$subj" == "$all_subjects" ]; then
                model_name="vis_roi_pretrained_lr_${lr}_subj_1257_pn_${pn}_cluster"
            else
                model_name="vis_roi_pretrained_lr_${lr}_subj_${subj}_pn_${pn}_cluster"
            fi
