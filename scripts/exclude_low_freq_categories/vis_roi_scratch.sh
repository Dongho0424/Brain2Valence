...
do
    ...
    do
        for subj in 1 2 5 7 "$all_subjects"1
        do
            for pn in 1024 2048 4096
            do
                if [ "$subj" == "$all_subjects" ]; then
                    model_name="vis_roi_scratch_lr_${lr}_subj_1257_pn_${pn}_exclude_low_st${st}"
                else
                    model_name="vis_roi_scratch_lr_${lr}_subj_${subj}_pn_${pn}_exclude_low_st${st}"
                fi
