...
do
    model_name="img_only_scratch_lr_${lr}"
    group="img_only_scratch"
    note="250116_1"

    CUDA_VISIBLE_DEVICES=${device} python3 -W "ignore" -W "ignore" main.py --exec_mode train --subj ${subj} \
    ...
    CUDA_VISIBLE_DEVICES=${device} python3 -W "ignore" -W "ignore" main.py --exec_mode predict --subj ${subj} \
    --model-name $model_name --notes $note --group $group --wandb-project $project --wandb-entity "donghochoi" \
    --epochs 50 --batch-size 52 --lr ${lr} --weight-decay $wd --optimizer adamw --scheduler cosine --criterion emotic_SL1 --cat-criterion ${cat_loss} \
    --task-type emotic --pretrained None --image-backbone resnet18 --model-type ${model_type} --data brain3d --with-nsd --cat-only \
    --best --wandb-log
